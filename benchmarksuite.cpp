...
static void BM_unorderedmap_insert_res(benchmark::State& state) {
  std::unordered_map<uint32_t,uint32_t> h;
  h.reserve(16777216);
  std::mt19937 gen(100);
  std::uniform_int_distribution<uint32_t> uni(0, std::numeric_limits<uint32_t>::max());
...
static void BM_memmaprosetta_arm(benchmark::State& state) {
  ...
  long x;
  for (auto _: state) {
    x = memmaprosetta_arm(state.range(0));
  }
  return;
}
...
//BENCHMARK(BM_unorderedmap_insert_res) -> Name("unorderedmap_insert_res") ->Arg(10000)->Arg(100000)->Arg(1000000)->Arg(10000000)->Arg(100000000) -> Unit(benchmark::kMillisecond);
